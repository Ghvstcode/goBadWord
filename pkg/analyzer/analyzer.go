...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	for _, file := range pass.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			//for _, sValx := range val {
			//	fmt.Println("sVALx-", sValx)
			//	v, ok := sValx[n.Pos()]
			//	if ok {
...
func (v *treeVisitor) Visit(n ast.Node) ast.Visitor {
	...
	case *ast.GenDecl:
		v.genericCheckAndAdd(n.Tok.String(), n.Pos())
	case *ast.IfStmt:
		if n.Pos() != token.NoPos {
	...
	case *ast.ValueSpec:
		for _, vs := range n.Names{
			v.genericCheckAndAdd(vs.String(), n.Pos())
		}
	case *ast.FuncType:
		v.FuncType(n)
	default:
...
func (v *treeVisitor) BasicLit(n *ast.BasicLit) {
	vl := strings.Trim(n.Value, "")
	v.genericCheckAndAdd(vl, n.Pos())
}